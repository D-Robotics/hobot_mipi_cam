...
int icm42688::read(ImuData_T *data) {
    ...
    // 转换为实际物理值
    if (accel_read) {
        data->ax = accel_x_raw * accel_scale * M_PI / 180.0;
        data->ay = accel_y_raw * accel_scale * M_PI / 180.0;
        data->az = accel_z_raw * accel_scale * M_PI / 180.0;
    } else {
        data->ax = 0.0f;
        data->ay = 0.0f;
        data->az = 0.0f;
    ...
    if (gyro_read) {
        data->gx = gyro_x_raw * gyro_scale * gravity;
        data->gy = gyro_y_raw * gyro_scale * gravity;
        data->gz = gyro_z_raw * gyro_scale * gravity;
    } else {
        data->gx = 0.0f;
        data->gy = 0.0f;
        data->gz = 0.0f;
    ...
    if (dtpkt.fsync_success) {
       ...
       // printf("[timestamp_data]%ld\n", dtpkt.accel_data_processed.timestamp);
        data->ax = dtpkt.accel_data_processed.x * M_PI / 180.0;
        data->ay = dtpkt.accel_data_processed.y * M_PI / 180.0;
        data->az = dtpkt.accel_data_processed.z * M_PI / 180.0;
        data->gx = dtpkt.gyro_data_processed.x * gravity;
        data->gy = dtpkt.gyro_data_processed.y * gravity;
        data->gz = dtpkt.gyro_data_processed.z * gravity;
